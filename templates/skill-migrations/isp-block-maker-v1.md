...
## Route reports and image display by user intent

Use the ISP Block Maker workspace tools and the following defaults without asking the user to choose a destination again:

- **“리포트 발행해”, “보고서 만들어줘”, “시각화해”, “결과를 비교해서 보여줘” → Visualizations.** Produce a self-contained HTML report (or a suitable supported image), register it with `artifact FILE --block BLOCK_ID --revision N --title "Report title"`, then call `present --artifact ARTIFACT_ID` using the saved artifact ID. Choose a relevant existing block; for a graph-wide report, describe the whole graph and associate it with a relevant output/entry block rather than inventing a processing node. Read `project` for the current revision and verify the registered artifact. Do not finish with only a chat summary, a local file path, an unregistered HTML file, or an image opened in Image Viewer. In this context “발행” means registering in the local Visualizations tab, not publishing externally. Report content must reflect actual outputs and validation; distinguish illustrative examples from measured results.
- **“뷰어에 띄워”, “이미지 뷰어에서 열어”, “이 RAW 보여줘” → Image Viewer.** Read `viewer.md` and use `viewer-open FILE` (RAW: add `--spec SPEC_JSON`) to register/reuse the image and request display. Confirm `displayed` / command acknowledgement; importing alone is not display. Keep the source image and known render conditions, and ask only for missing RAW metadata or an unclear target file. Do not substitute a Visualizations report or request a crop unless the user asks for one.

An explicitly named destination overrides these defaults. An explicit SDD / Software Development Document / Documentation request follows `sdd.md`; a generic report request does not automatically become an SDD. If an HTML report is explicitly requested in Image Viewer, explain the format mismatch and use Visualizations for the HTML, offering image output only when appropriate. When both analysis/report and source inspection are requested, register the report in Visualizations and open the requested source in Image Viewer; use the user's final display instruction for the foreground tab. Do not repeatedly switch tabs back and forth.

Keep draft/intermediate files in `tmp/<task-or-job-id>/` per `temporary-files.md`; register final results through the artifact tool. A failed registration or presentation is not completion: fix it or state the specific limitation. With no connected screen or blocked presentation, report the saved result and pending display accurately. Never discard unsaved user edits to force either view open.
