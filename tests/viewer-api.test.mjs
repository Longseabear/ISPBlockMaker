...
test("Viewer authenticates imports, presents requests, persists exact user crops and rejects resubmission",{timeout:60000},async()=>{
 ...
 try{
  ...
  assert.equal((await post("/viewer/images/import",{path:path.resolve("package.json"),spec})).status,400);
 ...
 }finally{socket?.terminate();child.kill();await exited;fs.rmSync(temp,{recursive:true,force:true});}
});
