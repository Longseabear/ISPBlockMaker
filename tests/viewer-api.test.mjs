...
test("Viewer authenticates imports, presents requests, persists exact user crops and rejects resubmission",{timeout:60000},async()=>{
 ...
 try{
  ...
  const groupImage=await(await post("/viewer/images/import",{path:path.join(temp,"input.raw"),spec:{...spec,group:1}})).json();
  const groupRequest=await(await post("/viewer/requests",{imageId:groupImage.id,prompt:"one grouped crop",show:false})).json();
  const groupedId=crypto.randomUUID(),rois=[{x:0,y:0,width:4,height:4},{x:4,y:4,width:4,height:4}];
  const invalidGroup=await post(`/viewer/requests/${groupRequest.id}/crops`,{id:crypto.randomUUID(),rois:[rois[0],{x:1,y:0,width:4,height:4}]});assert.equal(invalidGroup.status,400);
  assert.equal((await(await fetch(base+`/api/viewer/requests/${groupRequest.id}`,{headers})).json()).crops?.length||0,0);
  const groupedResponse=await post(`/viewer/requests/${groupRequest.id}/crops`,{id:groupedId,rois,description:"compare both regions"});assert.equal(groupedResponse.status,201);
  const grouped=await groupedResponse.json(),group=grouped.crops[0];assert.equal(grouped.crops.length,1);assert.equal(group.regions.length,2);assert.deepEqual(group.regions.map(r=>r.roi),rois);
  assert.equal((await(await post(`/viewer/requests/${groupRequest.id}/crops`,{id:groupedId,rois})).json()).crops.length,1);
  ...
  const bundleRoot=fs.mkdtempSync(path.join(os.tmpdir(),'isp-group-share-'));
  try{const bundle=path.join(bundleRoot,'group.bundle'),restored=path.join(bundleRoot,'restored');await packBundle(temp,bundle);await unpackBundle(bundle,restored);for(const region of group.regions)assert.deepEqual(fs.readFileSync(path.join(restored,region.paths.crop)),regionBytes(region.roi));}finally{fs.rmSync(bundleRoot,{recursive:true,force:true});}
  const groupRemoved=await fetch(base+`/api/viewer/requests/${groupRequest.id}/crops/${groupedId}`,{method:'DELETE',headers,body:'{}'});assert.equal(groupRemoved.status,200);
  for(const file of new Set([Object.values(group.paths),...group.regions.map(r=>Object.values(r.paths))].flat()))assert.equal(fs.existsSync(path.join(temp,file)),false);
  ...
  const selectedRead=JSON.parse(execFileSync(process.execPath,[path.join(temp,'.isp/tools/isp.mjs'),'viewer-crops'],{cwd:temp,windowsHide:true,encoding:'utf8'}));assert.equal(selectedRead.crops[0].description,'평탄 영역 테스트');assert.equal(selectedRead.description,selection.description);
  ...
  const remainingSelection=await(await fetch(base+'/api/viewer/crop-selection',{headers})).json();assert.equal(remainingSelection.crops.length,1);assert.equal(remainingSelection.missing.length,1);assert.equal(remainingSelection.description,selection.description);
