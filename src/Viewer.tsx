...
import {X, Info, Save, Trash2, Download, FileJson} from "lucide-react";
...
function CropCard({crop,requestId,index,api,onSaved,onSelect,onDownload,chosen,onChoose}:{chosen:boolean;onChoose:(shift:boolean,toggle:boolean)=>void;crop:CropResult;requestId:string;index:number;api:Api;onSaved:()=>Promise<void>;onSelect:()=>void;onDownload:(kind:string)=>void}){
...
export function Viewer({api,token,requestId,commandId,onExpand}:{api:Api;token:string;requestId:string;commandId:string;onExpand:()=>void}){
 ...
 const captureBusy=useRef(false),roiEdited=useRef(false);
 const [chosenCrops,setChosenCrops]=useState<string[]>([]),[sendingCrops,setSendingCrops]=useState(false),[sentCrops,setSentCrops]=useState('');
 ...
 useEffect(()=>{setChosenCrops([]);setSentCrops('');cropAnchor.current=null;},[image?.id]);
 function chooseCrop(index:number,shift:boolean,toggle:boolean){const key=cropKey(cropItems[index]);setSentCrops('');
 ...
 async function sendCrops(){if(sendingCrops||!image||!chosenItems.length)return;setSendingCrops(true);setError('');try{await api('/viewer/crop-selection',{imageId:image.id,items:chosenItems.map(i=>({requestId:i.requestId,cropId:i.crop.id||i.requestId}))});setSentCrops(`${chosenItems.length}개 전달됨 · 에이전트에게 선택한 크롭을 봐달라고 요청하세요.`);}catch(e){setError(String(e));}finally{setSendingCrops(false);}}
 ...
 useEffect(()=>{
   ...
   const wheel=(event:WheelEvent)=>{
     if(!event.deltaY||(event.target as HTMLElement).closest(".viewer-info-float"))return;
 ...
 function currentView():ViewState|null {const v=viewport.current,c=canvas.current;if(!image||!preview||!v||!c||loadedKey!==expectedKey||preview.key!==expectedKey||!v.clientWidth||!v.clientHeight)return null;
   ...
   return{imageId:image.id,render:{mode,gamma,...displayLevels},zoom,area:preview.area,visible,highlights,...(showSelection?{selection:roi}:{})};}
 ...
 function clearSelection(){start.current=null;pan.current=null;roiEdited.current=false;setShowSelection(false);}
 ...
 function move(e:React.PointerEvent<HTMLDivElement>){if(pan.current&&viewport.current){setOffset({x:pan.current.left+e.clientX-pan.current.x,y:pan.current.top+e.clientY-pan.current.y});return;}if(!start.current||!image)return;const p=point(e),x=Math.min(p.x,start.current.x),y=Math.min(p.y,start.current.y);alignSelection({x,y,width:Math.max(1,Math.abs(p.x-start.current.x)),height:Math.max(1,Math.abs(p.y-start.current.y))});}
 ...
 async function capture(next:ROI){if(!image||!active||!showSelection||captureBusy.current)return;captureBusy.current=true;setBusy(true);setError("");try{
 ...
 function finish(e:React.PointerEvent<HTMLDivElement>){if(pan.current){pan.current=null;return;}const origin=start.current;start.current=null;if(!origin||!image)return;if(Math.hypot(e.clientX-origin.clientX,e.clientY-origin.clientY)<3){clearSelection();return;}const p=point(e);alignSelection({x:Math.min(p.x,origin.x),y:Math.min(p.y,origin.y),width:Math.max(1,Math.abs(p.x-origin.x)),height:Math.max(1,Math.abs(p.y-origin.y))});}
 ...
 return <section className="viewer work-board" onKeyDown={e=>{if(e.key==="Escape"){e.preventDefault();e.stopPropagation();clearSelection();}}}><header className="viewer-toolbar"><h2>Image Viewer</h2><button onClick={onExpand}>Expand viewer</button><select disabled={busy} aria-label="Viewer image" value={image?.id||""} onChange={e=>{setSelected("");setImageId(e.target.value);}}><option value="">이미지 선택</option>{state.images.map(i=><option key={i.id} value={i.id}>{i.name}</option>)}</select><select disabled={busy} aria-label="Crop request" value={selected} onChange={e=>setSelected(e.target.value)}><option value="">크롭 요청 선택</option>{state.requests.slice().reverse().map(r=><option key={r.id} value={r.id}>{r.status} · {r.prompt.slice(0,65)}</option>)}</select></header>

 {error&&<p role="alert">{error}</p>}
 {image&&<><div className="viewer-toolbar"><select aria-label="Preview mode" value={mode} onChange={e=>setMode(e.target.value)}><option value="color">Cell average</option><option value="gray">Grayscale</option>{image.spec.format==="raw"&&<><option value="cfa">Bayer / Tetra · CFA colors</option><option value="simple">Simple ISP · Demosaic + Gamma</option></>}</select>{mode==="simple"&&image.spec.format==="raw"&&<label>Gamma<input aria-label="Simple ISP gamma" type="number" min="0.1" max="5" step="0.1" value={gamma} onChange={e=>{const v=Number(e.target.value);if(v>=.1&&v<=5)setGamma(v);}}/></label>}{mode==="cfa"&&image.spec.format==="raw"&&<>{(["x","y"] as const).map(k=><label key={k}>View {k}<input type="number" min="0" max={(k==="x"?image.spec.width:image.spec.height)-1} value={viewOrigin[k]} onChange={e=>{const v=Number(e.target.value);if(Number.isInteger(v)&&v>=0&&v<(k==="x"?image.spec.width:image.spec.height))setViewOrigin(o=>({...o,[k]:v}));}}/></label>)}</>}<button onClick={()=>zoomCenter(Math.max(.1,zoom/1.5))}>−</button><span>{Math.round(zoom*100)}%</span><button onClick={()=>zoomCenter(Math.min(8,zoom*1.5))}>+</button><button onClick={fitPreview}>Fit</button><button disabled={!showSelection||highlights.length>=20} onClick={()=>setHighlights(h=>[...h,{...roi,label:`영역 ${h.length+1}`}])}>선택 영역 강조</button><button disabled={!highlights.length} onClick={()=>setHighlights([])}>강조 지우기</button><details className="viewer-tone"><summary>표시 레벨</summary><div><label>Black<input type="number" value={levels.black} onChange={e=>setLevels({...levels,black:Number(e.target.value)})}/></label><label>White<input type="number" value={levels.white} onChange={e=>setLevels({...levels,white:Number(e.target.value)})}/></label><button onClick={()=>setDisplayLevels(levels)}>Apply preview</button></div></details></div>

 <div className="viewer-content" style={{"--viewer-side-width":`${sideWidth}px`} as CSSProperties}><div className="viewer-image-column"><div className="viewer-canvas" ref={viewport} tabIndex={0} aria-label="이미지 영역" onContextMenu={e=>{if(!(e.target as HTMLElement).closest(".viewer-info-float"))e.preventDefault();}} onPointerDown={e=>{if((e.target as HTMLElement).closest('.viewer-info-float'))return;if(!preview)return;e.currentTarget.focus({preventScroll:true});if(e.button===0&&!e.shiftKey&&e.target===e.currentTarget){clearSelection();return;}if(e.shiftKey||e.button===1||e.button===2){pan.current={x:e.clientX,y:e.clientY,left:offset.x,top:offset.y};e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);return;}if(!active||busy||e.button!==0||e.target!==canvas.current)return;start.current={...point(e),clientX:e.clientX,clientY:e.clientY};e.currentTarget.setPointerCapture(e.pointerId);}} onPointerMove={move} onPointerUp={finish} onLostPointerCapture={()=>{if(start.current)clearSelection();pan.current=null;}} onPointerCancel={()=>{if(start.current)clearSelection();pan.current=null;}}>{preview?<canvas ref={canvas} aria-label="Crop selection canvas" width={preview.width} height={preview.height} style={{width:preview.width*zoom,height:preview.height*zoom,left:offset.x,top:offset.y}}/>:<p>미리보기 준비 중…</p>}<details className="viewer-info-float"><summary title="이미지 정보 · 에이전트 안내" aria-label="이미지 정보 · 에이전트 안내"><Info size={16}/><span>정보{command?.message||request?" · 안내":""}</span></summary><div><span>{image.spec.width} × {image.spec.height} · {image.spec.bitDepth}bit {image.spec.format==="raw"?`· ${image.spec.pattern} · group ${image.spec.group}`:"RGB"}</span> {command?.message&&<div className="viewer-request"><strong>에이전트 안내 · {command.message}</strong></div>}{request&&<div className="viewer-request"><strong>{request.status} · {request.prompt}</strong></div>}<details className="viewer-open-settings"><summary>이미지 열기 조건</summary><dl><dt>파일</dt><dd>{image.name}</dd><dt>크기 / 형식</dt><dd>{image.spec.width} × {image.spec.height} · {image.spec.format}</dd>{image.spec.format==='raw'&&<><dt>저장 / 유효 비트</dt><dd>16-bit little-endian · {image.spec.bitDepth}-bit · {image.spec.alignment.toUpperCase()} aligned</dd><dt>CFA 배열</dt><dd>{image.spec.pattern} · {image.spec.group===1?'Bayer':image.spec.group===2?'Tetra':'TetraSquare'} ({image.spec.group}×{image.spec.group})</dd><dt>헤더 / 행 간격</dt><dd>{image.spec.offset} bytes / {image.spec.stride} bytes</dd><dt>CFA 시작 위상</dt><dd>({image.spec.originX||0}, {image.spec.originY||0})</dd></>}<dt>현재 렌더링</dt><dd>{image.spec.format!=='raw'?'RGB':mode==='simple'?'Simple ISP':mode==='cfa'?'CFA colors':mode==='gray'?'Grayscale':'Cell average'}</dd><dt>Black / White</dt><dd>{displayLevels.black} / {displayLevels.white}</dd>{mode==='simple'&&image.spec.format==='raw'&&<><dt>Gamma</dt><dd>{gamma}</dd><dt>Demosaic</dt><dd>동일 색 {image.spec.group}×{image.spec.group} 평균 → 위상별 bilinear 보간 → 감마. WB / CCM 없음.</dd></>}</dl><small>파일 등록 시 적용한 해석 설정입니다. 파일 자체에서 검증한 센서 메타데이터는 아닙니다.</small></details></div></details></div>
</div>
 ...
 <aside className="viewer-crop-list" aria-label="크롭 목록"> {importForm}<section className="viewer-share"><h3>현재 View</h3><small role="status">{liveStatus}</small><p>에이전트에게 “지금 보고 있는 부분을 봐줘”라고 요청하세요. 화면과 좌표·렌더링 설정을 최신 상태로 공유합니다.</p></section> <div className="viewer-toolbar viewer-roi">{(["x","y","width","height"] as const).map(key=><label key={key}>{key}<input disabled={!active||busy} aria-label={`Crop ${key}`} type="number" value={roi[key]} onChange={e=>{roiEdited.current=true;setRoi({...roi,[key]:Number(e.target.value)});}} onBlur={()=>{if(roiEdited.current){roiEdited.current=false;alignSelection(roi);}}}/></label>)}<span>{image.spec.format==="raw"?`CFA alignment required · ${2*image.spec.group}×${2*image.spec.group}`:"RGB pixel selection"}</span>
 <button disabled={busy||!preview||!active||!showSelection} onClick={()=>void capture(roi)}>크롭 추가</button><button className="viewer-icon-button" title="크롭 선택 취소 (Esc)" aria-label="크롭 선택 취소" disabled={!showSelection} onClick={clearSelection}><X size={16}/></button>{request?.status==="pending"&&<button disabled={busy} onClick={()=>action(`/viewer/requests/${request.id}/cancel`,{})}>Cancel request</button>}<small>{busy?"크롭 저장 중…":"영역 선택 후 크롭 추가를 누르세요."}</small></div><h3>크롭 영역 <span>{cropItems.length}</span></h3>{cropItems.length>0&&<div className="viewer-crop-batch"><small>Shift+클릭: 범위 선택 · 체크박스: 개별 선택<br/>설명은 저장 아이콘을 누른 내용만 전달됩니다.</small><button disabled={sendingCrops||!chosenItems.length} onClick={()=>void sendCrops()}>선택한 크롭 전달 ({chosenItems.length})</button>{sentCrops&&<small role="status">{sentCrops}</small>}</div>}{!cropItems.length&&<p>이미지에서 드래그한 뒤 크롭 추가를 누르세요. 여러 영역을 모으고 각각 설명을 남길 수 있습니다.</p>}{cropItems.map(({crop,requestId:owner},index)=><CropCard key={`${owner}:${crop.id||owner}`} crop={crop} requestId={owner} index={index} chosen={chosenCrops.includes(`${owner}:${crop.id||owner}`)} onChoose={(shift,toggle)=>chooseCrop(index,shift,toggle)} api={api} onSaved={refresh} onSelect={()=>{setShowSelection(true);setRoi(crop.roi);if(mode==="cfa")setViewOrigin({x:crop.roi.x,y:crop.roi.y});}} onDownload={kind=>void download(owner,crop,kind)}/>)}</aside></div>
 <details className="viewer-help"><summary>좌표 / 미리보기 안내</summary>
 <small>휠로 마우스 위치를 중심으로 확대·축소합니다. Esc 또는 × 버튼으로 크롭 선택을 취소합니다. 이미지나 여백을 클릭해도 해제됩니다. 오른쪽 버튼 + 드래그로 이미지를 이동합니다. Shift + 드래그와 가운데 버튼도 사용할 수 있습니다. 좌표는 원본 픽셀 기준, 좌상단 (0, 0)입니다. CFA colors는 원본 픽셀의 R/G/B 채널만 표시합니다. 최대 1200×1200 원본 영역이며 View x/y로 이동합니다. Simple ISP는 동일 색 그룹 평균 → 보간 → 감마(기본 2.2) 미리보기이며 색 보정·화이트밸런스는 하지 않습니다. RAW 크롭 값은 변경하지 않습니다. RAW는 드래그와 숫자 입력 모두 완전한 CFA 셀로 정렬합니다. 숫자 입력은 포커스를 옮기면 정렬되며 가장자리의 불완전한 셀은 제외됩니다. end 좌표는 영역에 포함되지 않습니다.</small></details></>}
 {!image&&<>{importForm}<div className="sdd-empty">이미지를 열거나 에이전트의 크롭 요청을 선택하세요.</div></>}
 </section>;
}
