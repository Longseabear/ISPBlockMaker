...
import {Save, Trash2, Download, FileJson, RotateCcw} from "lucide-react";
...
type SavedView={id:string;note:string;createdAt:string;view:ViewState;paths:{image:string;metadata:string}};
...
export function Viewer({api,token,requestId,commandId,onExpand}:{api:Api;token:string;requestId:string;commandId:string;onExpand:()=>void}){
 ...
 const [highlights,setHighlights]=useState<Highlight[]>([]),[note,setNote]=useState(''),[savedViews,setSavedViews]=useState<SavedView[]>([]),[sharing,setSharing]=useState(false),[shared,setShared]=useState('');
 ...
 function currentView():ViewState|null {const v=viewport.current,c=canvas.current;if(!image||!preview||!v||!c||loadedKey!==expectedKey||preview.key!==expectedKey)return null;const a=preview.area,sx=a.width/c.clientWidth,sy=a.height/c.clientHeight;return{imageId:image.id,render:{mode,gamma,...displayLevels},zoom,area:a,visible:{x:a.x+v.scrollLeft*sx,y:a.y+v.scrollTop*sy,width:Math.min(v.clientWidth,c.clientWidth-v.scrollLeft)*sx,height:Math.min(v.clientHeight,c.clientHeight-v.scrollTop)*sy},highlights,...(showSelection?{selection:roi}:{})};}
 liveView.current=currentView;
 useEffect(()=>{const timer=setInterval(()=>{const view=liveView.current();if(view)void api('/viewer/view',{...view,sessionId:sessionId.current}).catch(()=>{});},1500);return()=>clearInterval(timer);},[api]);
 async function loadViews(){const result=await api<{snapshots:SavedView[]}>('/viewer/view');setSavedViews(result.snapshots);}
 useEffect(()=>{void loadViews().catch(e=>setError(String(e)));},[api]);
 async function shareView(){const view=currentView(),c=canvas.current,v=viewport.current;if(!view||!c||!v)return;setSharing(true);setError('');try{const sx=(view.visible.x-view.area.x)/view.area.width*c.width,sy=(view.visible.y-view.area.y)/view.area.height*c.height,sw=view.visible.width/view.area.width*c.width,sh=view.visible.height/view.area.height*c.height;const scale=Math.min(zoom,2048/Math.max(sw,sh));const out=document.createElement('canvas');out.width=Math.max(1,Math.round(sw*scale));out.height=Math.max(1,Math.round(sh*scale));out.getContext('2d')!.drawImage(c,sx,sy,sw,sh,0,0,out.width,out.height);const result=await api<SavedView>('/viewer/views',{view,note,png:out.toDataURL('image/png')});setShared(result.id);setNote('');await loadViews();}catch(e){setError(String(e));}finally{setSharing(false);}}
 ...
 <div className="viewer-content"><div className="viewer-image-column"><div className="viewer-canvas" ref={viewport}>{preview?<canvas ref={canvas} aria-label="Crop selection canvas" width={preview.width} height={preview.height} style={{width:preview.width*zoom,height:preview.height*zoom}} onPointerDown={e=>{if(e.shiftKey||e.button===1){const v=viewport.current!;pan.current={x:e.clientX,y:e.clientY,left:v.scrollLeft,top:v.scrollTop};e.preventDefault();e.currentTarget.setPointerCapture(e.pointerId);return;}if(!active||busy||e.button!==0)return;start.current={...point(e),clientX:e.clientX,clientY:e.clientY};e.currentTarget.setPointerCapture(e.pointerId);}} onPointerMove={move} onPointerUp={finish} onLostPointerCapture={()=>{start.current=null;pan.current=null;}} onPointerCancel={()=>{start.current=null;pan.current=null;}}/>:<p>미리보기 준비 중…</p>}</div>
