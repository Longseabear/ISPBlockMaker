...
export function JobsBoard({project,onOpen,api,onArtifact}:{project:Project;api:Api;onArtifact:(id:string)=>void;onOpen:(id:string|null)=>void}){
 ...
 return <section className="work-board"><header><h1>JOB Queue</h1><p>전체 그래프와 모든 블록의 작업 · 남음 {remaining.length} · 진행 중 {jobs.filter(j=>j.status==='in_progress').length}</p></header><div className="work-controls"><button aria-pressed={tab==='jobs'} onClick={()=>setTab('jobs')}>작업 목록</button><button aria-pressed={tab==='activity'} onClick={()=>setTab('activity')}>활동 기록</button></div>{tab==='activity'?<ActivityLog api={api} project={project} onOpen={onOpen} onArtifact={onArtifact}/>:<><div className="work-controls"><input aria-label="JOB 검색" placeholder="블록·요청·작업 검색" value={query} onChange={e=>setQuery(e.target.value)}/><select aria-label="JOB 상태" value={filter} onChange={e=>setFilter(e.target.value)}><option value="remaining">남은 JOB</option><option value="pending">대기</option><option value="in_progress">진행 중</option><option value="done">완료</option><option value="all">전체</option></select></div>
